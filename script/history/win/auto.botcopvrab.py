...
def find_item_names(frame_bgr: np.ndarray) -> List[Dict[str, Any]]:
    ...
    keep = merge_same_lines(boxes_scores, line_thr=ITEM_GROUP_STEP)
...
def pickup_state(frame_bgr: np.ndarray, card_rect: Optional[Tuple[int, int, int, int]] = None) -> Dict[str, Any]:
    ...
    if best_templ["kind"] == "inactive" and templ_score >= PICKUP_INACTIVE_THRESHOLD:
        ...
        state = "active"
    elif mixed < PICKUP_INACTIVE_THRESHOLD:
        state = "inactive"
    else:
        return {
            "state": "out_of_view",
...
def auto_loot_once():
    ...
    while idx < len(items):
        ...
        st = swipe_until_visible_pickup(get_frame_fn=get_frame, card_rect=None)
