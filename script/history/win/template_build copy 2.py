...
def normalize_ru(s: str) -> str:
    # нижний регистр, ё→е, убираем пунктуацию кроме пробелов/дефиса, схлопываем пробелы
    s = s.strip().lower()
    s = s.replace("ё", "е")
    s = NON_WORDS.sub(" ", s)
...
def measure_text(draw: ImageDraw.ImageDraw, text: str, font: ImageFont.ImageFont) -> Tuple[int, int]:
    # Совместимость с разными версиями Pillow: textbbox → getsize → fallback
    try:
...
def load_item_names(narrator: Narrator, path: Path) -> Dict[str, str]:
    ...
    if not items:
        # Простой парсер: строки с '- ' или одна на строку
    ...
    if not items:
        ...
        sys.exit(1)
    # Нормализованная форма → оригинал
...
def group_into_lines(words: List[Word]) -> List[Line]:
    # Группируем естественным образом по (page, block, par, line), затем сортируем по left
    ...
    # Сортировка по вертикали, затем по x
...
def contains_phrase(hay_norm: str, needle_norm: str) -> bool:
    # Поиск по словоформам как подпоследовательность с границами слов
...
def match_line_to_item(line_norm: str, items_norm_to_orig: Dict[str, str]) -> Optional[str]:
    ...
    for inorm, orig in items_norm_to_orig.items():
        if contains_phrase(line_norm, inorm):
            return orig
    # 3) все слова needle присутствуют по порядку (на случай двойных пробелов/дефисов)
    tokens = line_norm.split()
    for inorm, orig in items_norm_to_orig.items():
        ntoks = inorm.split()
        if len(ntoks) == 0:
            continue
        # ищем непрерывный фрагмент
...
def process_image(
    ...
    # Чтоб не сохранять один и тот же предмет много раз с одного кадра:
    matched_items_in_image: Set[str] = set()

    for ln in lines:
        # Отсекаем совсем шумные строки
        if ln.conf < min_conf:
            # Можно сложить в rejects по желанию — тут пропустим
            continue

        match_name = match_line_to_item(ln.norm, items_norm_to_orig)
        if match_name is not None:
            if match_name in matched_items_in_image:
                # уже найден — пропускаем дубль
                continue
            ...
            # Сохраним информативные строки в rejects (можно выключить, если мешает)
            if ln.width > 0 and ln.height > 0 and len(ln.norm) >= 2:
