...
def find_item_names(frame_bgr: np.ndarray) -> List[Dict[str, Any]]:
    ...
    Возвращает список элементов:
      { name:str, score:float, center:(cx,cy), box:(x,y,w,h), slot_hash:int }
    slot_hash — хеш строки (имя + иконка) для уникальности дубликатов.
    """
    H, W = frame_bgr.shape[:2]
    x1, x2 = 0, min(500, W)
    ...
    for i in keep:
        ...
        row_crop = frame_bgr[ay : ay + h, ax : ax + w]
    ...
    try:
        ...
        pass
    log(f"[ITEMS] строк после группировки: {len(found)} (порог={ITEM_GROUP_THRESHOLD})")
    return found
...
def compute_row_hash(frame_bgr, box):
    x, y, w, h = box
    crop = frame_bgr[y : y + h, x : x + w]
    gray = to_gray(crop)
    return frame_hash(gray)


def verify_item_removed(name: str, base_hash: int) -> bool:
    """
    Ждём обновления UI и дважды перепроверяем, исчез ли слот (по имени + slot_hash).
    """
    time.sleep(1.8)
    for _ in range(2):
        frame = screenshot_bgr()
        items_after = find_item_names(frame) or []
        still_here = any(it["name"] == name and int(it["slot_hash"]) == int(base_hash) for it in items_after)
        if not still_here:
            return True
        time.sleep(0.8)
    return False
...
def auto_loot_once():
    ...
    while items:
        ...
        if base_hash in processed_hashes:
            items.pop(0)
            continue
        log(f"[ITEM] open '{name}' at {current['center']} y={current['box'][1]} hash={hex(base_hash)}")
        ...
        if frame_card is None:
            log("[ERROR] no frame for card; skip")
        ...
        st = swipe_until_visible_pickup(get_frame_fn=screenshot_bgr, card_rect=card_rect)
        state, box = st["state"], st["box"]
        log(f"[PICKUP] state={state} score={st['score']} method={st['method']}")
        if state == "inactive":
            if ABORT_ON_FIRST_INACTIVE:
                log(f"[FLOW] '{name}': кнопка неактивна — завершаем лут (политика)")
                collapse_item_panel((cx, cy))
                ensure_tab_state("вещи", "закрыта")
                log("=== AUTO-LOOT END ===")
                return
            else:
                log(f"[FLOW] '{name}': кнопка неактивна — пропуск")
                collapse_item_panel((cx, cy))
                processed_hashes.add(base_hash)
                items.pop(0)
                continue
        if state == "active":
            ...
            dbg_save(
                "pickup_click",
                frame_card,
                rects=[box] if box else [st["roi_rect"]],
                points=[(bx, by)],
                color=(0, 255, 0),
            )
            tap(bx, by, reason="pickup")
            if verify_item_removed(name, base_hash):
                log(f"[FLOW] '{name}' подобран — подтверждено")
                processed_hashes.add(base_hash)
            else:
                log(f"[WARN] '{name}' не исчез после двух проверок — помечаем обработанным (антизацикливание)")
                processed_hashes.add(base_hash)
            collapse_item_panel((cx, cy))
        if state == "out_of_view":
            log(f"[FLOW] '{name}': кнопка не в видимой зоне после свайпов — пропуск")
            collapse_item_panel((cx, cy))
            processed_hashes.add(base_hash)
        frame = screenshot_bgr()
        items = find_item_names(frame) or []
        items = [it for it in items if int(it["slot_hash"]) not in processed_hashes]
        items.sort(key=lambda d: d["box"][1])
